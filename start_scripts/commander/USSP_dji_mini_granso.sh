#!/bin/bash
app=../../../rise_drones/src/app/app_ussp_mission.py
mission=../../mission/missions/USSP_Granso_dji_mini.json
echo "app: $app"
echo "mission: $mission"
#/home/droneadmin/.venv/rise_drones/bin/python3 $app --crm=10.44.160.10:16300 --app_ip=10.44.160.10 --mission=$mission --stdout --log=info --capabilities C0
python3 $app --crm=10.44.160.10:16300 --app_ip=10.44.163.70 --mission=$mission --stdout --log=info --capabilities C0
